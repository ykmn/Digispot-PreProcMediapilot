...
$headers = 'Time','s1','s2','ID_number','s','s4','s5','Dur','s6','Dur2','s8','s9','s10','s11','s12','s13','Date','Name'
...
$array = Import-Csv $csvFile -Delim ';' -Encoding "UTF8" `
    -Header $headers


# SKIP - Set condition for skip
$array = $array | Where-Object { $_.ID_number -notlike "" }          # Skiping lines with empty IDNo

# SELECT - Select fields for output
$array | Select-Object Time,IDNo,Dur,Date,Name |                     # Selecting meaningful fields
    Export-Csv -Path $csvFile  -Delim ';' -Encoding "UTF8"           # Saving back to CSV as Windows-1251 -NoType
...
Remove-Item -Path $csvFile                                           # and removing temp file
Remove-Item -Path $utfFile